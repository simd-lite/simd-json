...
fn push_last_byte_of_a_to_b(a: __m128i, b: __m128i) -> __m128i {
    unsafe { _mm_alignr_epi8(b, a, 16 - 1) }
}

#[cfg_attr(not(feature = "no-inline"), inline)]
fn push_last_2bytes_of_a_to_b(a: __m128i, b: __m128i) -> __m128i {
    unsafe { _mm_alignr_epi8(b, a, 16 - 2) }
}
...
fn continuation_lengths(high_nibbles: __m128i) -> __m128i {
    unsafe { _mm_shuffle_epi8(nibbles_tbl!(), high_nibbles) }
}
...
fn check_overlong(
    ...
    unsafe {
        ...
        let initial_mins: __m128i = _mm_shuffle_epi8(initial_mins_tbl!(), off1_hibits);

        let initial_under: __m128i = _mm_cmpgt_epi8(initial_mins, off1_current_bytes);

        let second_mins: __m128i = _mm_shuffle_epi8(second_mins_tbl!(), off1_hibits);
